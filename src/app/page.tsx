...
import { Code2, Github, Download, Eye, Code, Clipboard, Check } from 'lucide-react';
...
export default function Home() {
  ...
  const [previewFiles, setPreviewFiles] = useState<Record<string, { code: string }>>({});
    ...
    try {
      ...
      if (jsonMatch && jsonMatch[1]) {
        ...
        if (projectData.type === 'new_project' && projectData.fileStructure && projectData.files) {
          ...
          if (projectData.fileStructure[0]?.children?.[0]?.children?.[0]?.path) {
            setSelectedFile(projectData.fileStructure[0].children[0].children[0].path);
          }
      ...
      <div className="flex flex-col h-screen">
        <header className="border-b border-claude-border py-3 px-4 flex justify-between items-center bg-white">
          <div className="flex items-center">
            ...
            <h1 className="text-xl font-semibold text-claude-text">CodeCraft AI</h1>
          </div>
